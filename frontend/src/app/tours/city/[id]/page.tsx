...
import { ArrowRight, MapPin, Clock, Calendar, Heart } from "lucide-react";
...
export default function CityToursPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const [cityRes, toursRes] = await Promise.all([
                    axios.get(`cities/${id}`),
                    axios.get("tours")
                ]);
                ...
                setTours(allTours.filter(t => t.city_id == id));
            } catch (error) {
        ...
        <main className="bg-white min-h-screen">
            ...
            {loading ? (
                <div className="flex flex-col items-center py-40">
                    ...
                    <span className="text-[10px] uppercase tracking-widest font-black text-black/20">Preparing Expeditions...</span>
                </div>
            ) : (
                    ...
                    <div className="container-luxury">
                        <div className="flex items-end justify-between border-b border-black/10 pb-8 mb-16">
                            <div>
                                ...
                                <h3 className="text-4xl font-light tracking-tighter uppercase text-black">Available Tours</h3>
                            </div>
                            <span className="text-xs font-black tracking-widest text-black/30 uppercase">{tours.length} Found</span>
                        </div>

                        {tours.length === 0 ? (
                            <div className="text-center py-20">
                                <p className="text-black/30 uppercase tracking-widest text-xs font-black">No expeditions currently charted for this territory.</p>
                                <Link href="/tours" className="inline-block mt-8 px-8 py-3 border border-black/10 text-black hover:border-gold hover:text-gold transition-colors text-[10px] uppercase tracking-[0.3em] font-black">
                                    Return to Directory
                                </Link>
                            </div>
                        ) : (
                                ...
                                <AnimatePresence mode="popLayout">
                                    {tours.map((tour, tIdx) => (
                                        <motion.div 
                                            ...
                                            animate={{ opacity: 1, y: 0 }}
                                            transition={{ duration: 0.8, delay: tIdx * 0.1, ease: [0.16, 1, 0.3, 1] }}
                                            className="group flex flex-col h-full"
                                        >
                                            <div className="aspect-[16/10] overflow-hidden rounded-sm relative mb-12 shadow-2xl">
                                                <img 
                                                    src={tour.images?.[0] || tour.image || "https://images.unsplash.com/photo-1544085311-11a028465b03"} 
                                                    alt={tour.title} 
                                                ...
                                                <div className="absolute top-10 left-10 glass-light px-6 py-3 rounded-full">
                                                    <span className="text-[11px] font-black tracking-[0.3em] uppercase">${tour.price}</span>
                                                </div>
                                                <button 
                                                    onClick={async (e) => {
                                                        e.preventDefault();
                                            ...
                                            <div className="space-y-8">
                                                <div className="flex items-center gap-8 text-[10px] uppercase tracking-[0.5em] text-black/30 font-black">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <span>{tour.location || city?.name}</span>
                                                    </div>
                                                    <div className="flex items-center gap-3">
                                                ...
                                                <Link href={`/tours/${tour.slug || tour.id}`} className="flex items-center gap-6 text-black font-black uppercase tracking-[0.5em] text-[11px] pt-12 group/btn border-t border-black/5 w-full">
                                                    Discover
